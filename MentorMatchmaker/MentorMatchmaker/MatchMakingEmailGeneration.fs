module MatchMakingEmailGeneration
...
let dumpMailToLinksInHtmlFile (mentorshipMatches: ConfirmedMentorshipApplication list) =
    ...
    let fileContent =
        mentorshipMatches
        |> List.map(fun confirmedMatch -> [ transformConfirmedMatchIntoMenteeEmailTemplate confirmedMatch; transformConfirmedMatchIntoMentorEmailTemplate confirmedMatch])
        |> List.concat
